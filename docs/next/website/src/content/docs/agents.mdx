...
When Codex cannot get capacity for the selected model it prints `⚠ Selected model is at capacity. Please try a different model.` as a transcript line and returns to its prompt. Herdr reports that pane as `at_capacity` while the warning is the last output above the live prompt. The state has its own sidebar icon and color, rolls up to the tab and Space like `blocked`, and sends a notification with the needs-input sound so you can retry or switch models. It clears as soon as the next turn starts or any other output appears.
